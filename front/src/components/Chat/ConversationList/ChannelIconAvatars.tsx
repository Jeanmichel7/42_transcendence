...
const ConversationListRoomItemIcons = ({
  conv,
}: ConversationListRoomItemIconsProps) => {
  const [usersToDisplay, setUsersToDisplay] = useState<UserInterface[] | null>(
    null,
  );
  ...
  const getRandAdmin = useCallback(() => {
    ...
    const admins: UserInterface[] = room.admins.filter(
      u => u.id !== room.ownerUser?.id,
    );
    const rand: number = Math.floor(Math.random() * 100) % admins.length;
    return admins[rand];
  }, [room]);

  const getRandUser = useCallback(
    (exceptedUser: UserInterface | null) => {
      if (
        !room ||
        !room.ownerUser ||
        ...
        return;
      let users: UserInterface[] = room.users.filter(
        u =>
          u.id !== room.ownerUser?.id && !room.admins?.some(a => a.id === u.id),
      );
      if (exceptedUser) users = users.filter(u => u.id !== exceptedUser.id);
  ...
  useEffect(() => {
    if (
      room &&
      room.ownerUser &&
      (usersToDisplay == null || usersToDisplay.length < 2)
    ) {
      ...
      if (adminSelected) {
        ...
        if (userSelected)
          setUsersToDisplay(prev =>
            prev ? [...prev, userSelected] : [userSelected],
          );
      } else {
        const userSelected: UserInterface | undefined = getRandUser(null);
        if (userSelected) {
          setUsersToDisplay([userSelected]);
          const userSelected2: UserInterface | undefined =
            getRandUser(userSelected);
          if (userSelected2)
            setUsersToDisplay(prev =>
              prev ? [...prev, userSelected2] : [userSelected2],
            );
        }
      }
    }
