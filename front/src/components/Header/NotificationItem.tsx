...
const NotificationItem = ({
  ...
  const handleAcceptFriendRequest = async (userToAccept: UserInterface) => {
    ...
    if ('error' in resAcceptRequest)
      dispatch(
        setErrorSnackbar(
          resAcceptRequest.error + resAcceptRequest.message
            ? ': ' + resAcceptRequest.message
            : '',
        ),
      );
    else {
  ...
  const handleDeclineFriendRequest = async (userToDecline: UserInterface) => {
    ...
    if (typeof resDeclineRequest === 'object' && 'error' in resDeclineRequest)
      dispatch(
        setErrorSnackbar(
          resDeclineRequest.error + resDeclineRequest.message
            ? ': ' + resDeclineRequest.message
            : '',
        ),
      );
    else {
  ...
  const handleMouseEnter = async () => {
    ...
    if (typeof resReadNotif === 'object' && 'error' in resReadNotif)
      dispatch(
        setErrorSnackbar(
          resReadNotif.error + resReadNotif.message
            ? ': ' + resReadNotif.message
            : '',
        ),
      );
