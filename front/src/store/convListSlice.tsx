...
const helperAddConversationList = (
  state: ChatState,
  item: UserInterface | RoomInterface,
) => {
  const newConversation: ConversationInterface = {
    id:
      state.conversationsList.length === 0
        ? 0
        : state.conversationsList[state.conversationsList.length - 1].id + 1,
  ...
  if (isRoomInterface(item)) {
    ...
    throw new Error(
      "item n'est ni de type RoomInterface, ni de type UserInterface",
    );
  }
...
const helperUpdateStatusUserConvList = (
  state: ChatState,
  item: UserStatusInterface[],
) => {
  item.forEach(userStatus => {
    state.conversationsList.forEach((conv: ConversationInterface) => {
      ...
      if (isRoomInterface(conv.room)) {
        //check ownerUser
        if (
          conv.room.ownerUser?.id === userStatus.id &&
          conv.room.ownerUser.id === userStatus.id
        ) {
...
const helperUpdateConversationList = (
  state: ChatState,
  item: RoomInterface,
) => {
  const indexConvToUpdate = state.conversationsList.findIndex(
    conv => conv.room.id === item.id,
  );
...
export const chatSlice = createSlice({
  ...
  reducers: {
    ...
    reduxRemoveConversationToList: (
      state,
      action: PayloadAction<{ convId: number; userId: number }>,
    ) => {
      // console.log('REDUX REMOVE CONV LIST', action.payload);
      const { convId, userId } = action.payload;
      state.conversationsList = state.conversationsList.filter(
        (conv: ConversationInterface) => conv.id !== convId,
      );
      localStorage.setItem(
        'conversationsList' + userId,
        JSON.stringify(state.conversationsList),
    ...
    reduxUpdateRoomConvList: (
      state,
      action: PayloadAction<{ item: RoomInterface; userId: number }>,
    ) => {
    ...
    reduxUpdatePrivateConvList: (
      state,
      action: PayloadAction<{ item: UserInterface; userId: number }>,
    ) => {
    ...
    reduxUpdateStatusUserConvList: (
      state,
      action: PayloadAction<{ item: UserStatusInterface[]; userId: number }>,
    ) => {
    ...
    reduxRemoveWaitingUserInRoom: (
      state,
      action: PayloadAction<{ roomId: number; userId: number }>,
    ) => {
      ...
      if (indexConvToUpdate !== -1) {
        state.conversationsList[indexConvToUpdate].room.acceptedUsers =
          state.conversationsList[indexConvToUpdate].room.acceptedUsers?.filter(
    ...
    reduxAddNotReadMP: (
      state,
      action: PayloadAction<{ userIdFrom: number; userId: number }>,
    ) => {
      const { userIdFrom, userId } = action.payload;
      const indexConvToUpdate = state.conversationsList.findIndex(
        conv => conv.user.id === userIdFrom,
      );
      if (indexConvToUpdate !== -1) {
