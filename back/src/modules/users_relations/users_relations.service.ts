...
export class UsersRelationsService {
  ...
  async getAllRelations(userId: bigint): Promise<UserRelationInterface[]> {
    ...
    .select(['user_relation.id', 'user_relation.relationType', 'user_relation.createdAt', 'user_relation.updatedAt'])
    .leftJoin('user_relation.user', 'user')
    .addSelect(['user.id', 'user.login', 'user.email', 'user.firstName', 'user.lastName', 'user.description', 'user.avatar', 'user.status'])
    .leftJoin('user_relation.userRelation', 'userRelation')
    .addSelect(['userRelation.id', 'userRelation.login', 'userRelation.email', 'userRelation.firstName', 'userRelation.lastName', 'userRelation.description', 'userRelation.avatar', 'userRelation.status'])
	...
	async getAllFriendsofUser(userId: bigint): Promise<UserInterface[]> {
		const userRelations: UserRelationEntity[] = await this.userRelationRepository.find({ 
  ...
  async getAllBlockedUsers(userId: bigint): Promise<UserInterface[]> {
	...
	async addFriend(userId: bigint, friendId: bigint): Promise<UserRelationInterface> {
		if(userId === friendId)
		...
		const userToUpdate: UserEntity = await this.userRepository.findOne({
			where: { id: userId },
			select: ["id", "login", "email", "firstName", "lastName", "description", "avatar", "status"],
		});
		if (!userToUpdate)
		...
		const relationExist: UserRelationEntity = await this.userRelationRepository.findOne({ 
			where: { user: {id: userId}, userRelation: {id: friendId} },
			select: ["id", "relationType"],
		});
    console.log(relationExist);
    if (relationExist?.relationType == 'friend')
      throw new BadRequestException(`User ${userId} already friend ${friendId}`);
    ...
    try {
      ...
      throw new BadRequestException(`User ${userId} can't block ${friendId}`, e);
    }
	}
  ...
  async deleteRelation(userId: bigint, friendId: bigint): Promise<Boolean> {
    if(userId === friendId)
      throw new BadRequestException(`User ${userId} can't delete himself as friend`);
    ...
    // check if relation already exist
    const relationExist: UserRelationEntity[] = await this.userRelationRepository.find({
      where: { user: {id: userId}, relationType: 'friend', userRelation: {id: friendId} },
      select: ["id"],
    });
    if (relationExist?.length === 0)
      throw new BadRequestException(`User ${userId} don't have ${friendId} as friend`);

    // delete relation in users_relation table
    const relationToDelete: UserRelationEntity = await this.userRelationRepository.findOne({
      where: { user: {id: userId}, relationType: 'friend', userRelation: {id: friendId} },
      select: ["id"],
    });
    if(!relationToDelete)
      throw new NotFoundException(`Relation between user ${userId} and user friend ${friendId} not found`);

    try {
      const deletedRelation = await this.userRelationRepository.remove(relationToDelete);
      if(deletedRelation?.id == undefined)
        return true;
      else
        return false;
    }
  ...
  async blockUser(userId: bigint, blockedId: bigint): Promise<UserRelationInterface> {
    if(userId === blockedId)
    ...
    if(!userBlocked)
      throw new NotFoundException(`User blocked with id ${blockedId} not found`);
    ...
    if (relationExist?.relationType === 'blocked')
      throw new BadRequestException(`User ${userId} already blocked ${blockedId}`);
    ...
    try {
      ...
      throw new BadRequestException(`User ${userId} can't block ${blockedId}`, e);
    }
  }
