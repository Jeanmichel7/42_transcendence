...
export class OwnerRoomGuard implements CanActivate {
    ...
    if (!request.user)
      throw new UnauthorizedException("Authorization error", "User not accurate");
    ...
    if( user.roomOwner.length === 0 )
      throw new ForbiddenException("Authorization error", "User is not the owner of any room");

    const roomId: bigint = request.params?.roomId;
    if (!roomId)
      throw new UnauthorizedException("Authorization error", `Room not accurate`);
    ...
    throw new ForbiddenException("Authorization error", `User ${request.user.login} is not the owner of this room`);
	}
