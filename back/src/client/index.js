let token =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEiLCJsb2dpbiI6ImpyYXNzZXI4NDMiLCJyb2xlIjoidXNlciIsImlhdCI6MTY4MTU2OTM1MywiZXhwIjoxNjgxNzQyMTUzfQ.IrtZ290PvAJiQe9hK9pq0K71ZmqNswGWwCjr45GlaLI';
...
function joinRoom1(userIdDest) {
  ...
  //connect to room
  socketEvent1.emit('joinPrivateRoom', {
    user1Id: getUser(),
    user2Id: userIdDest,
  });
...
function joinRoom2(userIdDest) {
  ...
  socketEvent2.emit('joinPrivateRoom', {
    user1Id: getUser(),
    user2Id: userIdDest,
  });
...
async function createChatRoom() {
  const res = await axios.post(
    'http://localhost:3000/chat/rooms/add',
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  console.log('res : ', res);
  if (res) console.log('new room created, roomId : ', res.data.id);
}
...
async function joinChatRoom(roomId) {
  let res = await axios.get(
    'http://localhost:3000/chat/rooms/' + roomId + '/join',
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
function sendChatMessage(roomId) {
  ...
  axios.post(
    'http://localhost:3000/chat/rooms/' + roomId + '/messages/add',
    {
